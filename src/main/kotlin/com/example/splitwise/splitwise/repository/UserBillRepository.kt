...
interface UserBillRepository : JpaRepository<UserBill, Long> {
    ...
    @Query(nativeQuery = true, value = "select * from user_bill where (user_id = :user_id or owner_id = :user_id) and payment_status = 'PENDING' and bill_status = 'PRESENT'")
    fun findUserPendingAndUserOwnerBills(@Param(value = "user_id") userId: Long): Iterable<UserBill>
}
